...
> **Purpose:** The single, opinionated rulebook for *how we write, name, branch, review, secure,
> and ship code* at Propulse AI. This is the "how" layer on top of the architecture decisions in
> [`ARCHITECTURE.md`](ARCHITECTURE.md), [`REPOSITORY_STRUCTURE.md`](REPOSITORY_STRUCTURE.md), and
> [`CLEAN_ARCHITECTURE.md`](CLEAN_ARCHITECTURE.md). Those documents decide *what the system is*;
> this one decides *how every engineer and AI agent contributes to it*.
>
...
boundaries, multi-tenancy, events, or layering it *restates and operationalizes* the canonical docs.
...
We do **not** prefix interfaces with `I`. A port is named for the *capability the domain needs*,
...
context and job — `kb.ingest`). Job *names* within a queue, if any, are also `kebab-case`.
...
- **`staging`** is promoted by tagging `vX.Y.Z` (semver). The *exact same built artifact*
...
Reviewers verify *all* of the following. Block on a "no"; nits are non-blocking suggestions
(prefix with `nit:`).
...
- [ ] **Caching:** hot reads (org config, AI-employee config, resolved identity, KB retrieval) cached in
  Redis with an **explicit TTL** *and* an **invalidation path** (event-driven on config change). No cache
...
- [ ] Public functions/ports have concise doc comments; the *why* lives in code/ADR, not the PR thread.
...
A change is **Done** only when *all* of the following are true. This is the master gate — reviewers
