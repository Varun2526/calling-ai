...
- An **ADR** (in [`adr/`](adr/)) is the full record of one architecturally significant
  ...
  append-only**: a decision is changed by _superseding_ it with a new ADR, never by editing the
  old one.
- This **Decision Log** is the _index_: one row per decision, pointing at the ADR. Use it to
...
Seeded from [`ARCHITECTURE.md`](ARCHITECTURE.md) §15. This is the _maintained_ version — update
...
6. **Superseding:** to reverse or replace a decision, write a _new_ ADR, set the old ADR's
